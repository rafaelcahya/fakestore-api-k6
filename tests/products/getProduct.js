import http from "k6/http";
import { check, group } from "k6";
import { BASE_URL, PRODUCT_URL, PRODUCT_ID, PRODUCT_ID_NOT_FOUND } from '../../utils/config.js';

export const options = {
    vus: 1,
    duration: "10s",
    thresholds: {
        http_req_duration: ["p(95)<1000"],
        'http_req_duration{group:::Get user found}':["p(95)<1000"], 
        'http_req_duration{group:::Get user not found}':["p(95)<1000"],
        'http_req_failed{group:::Get user found}':["rate<0.01"], 
        'http_req_failed{group:::Get user not found}':["rate<0.01"],
        checks: ["rate>0.25"],
        'checks{group:::Get user found}':["rate>0.25"], 
        'checks{group:::Get user not found}':["rate>0.25"],
    },
};

export default function () {
    group("Get product found", function () {
        let res = http.get(`${BASE_URL}/${PRODUCT_URL}/${PRODUCT_ID}`);
        check(res, {
            "Status 200": (r) => r.status === 200,
            "Response time < 1000ms": (r) => r.timings.duration < 1000,
        });
    })

    group("Get product not found", function () {
        let res = http.get(`${BASE_URL}/${PRODUCT_URL}/${PRODUCT_ID_NOT_FOUND}`);
        check(res, {
            "Status 200": (r) => r.status === 200,
            "Response time < 1000ms": (r) => r.timings.duration < 1000,
        });
    })
}
